-- 012_route_new_admins_to_onboarding.sql
-- Ensure admin auth signups start at onboarding step 1 after email verification.

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS onboarding_step_completed integer;

ALTER TABLE public.profiles
  ALTER COLUMN onboarding_step_completed SET DEFAULT 0;

UPDATE public.profiles
SET onboarding_step_completed = 0
WHERE onboarding_step_completed IS NULL;

ALTER TABLE public.profiles
  ALTER COLUMN onboarding_step_completed SET NOT NULL;
...
BEGIN
  ...
  DO UPDATE SET
    ...
    onboarding_step_completed = COALESCE(
      public.profiles.onboarding_step_completed,
      EXCLUDED.onboarding_step_completed
    );
