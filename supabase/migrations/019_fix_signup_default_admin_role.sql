-- 019_fix_signup_default_admin_role.sql
-- Self-signup default role: admin. Staff-created users can still set worker/manager via metadata.role.
-- No auth.users trigger DDL in this migration.
...
DECLARE
  ...
  v_requested_role text := lower(COALESCE(v_meta->>'role', ''));
  v_first_name text := '';
  v_last_name text := '';
  v_phone text := NULL;
  v_company_id uuid;
...
BEGIN
  ...
  IF v_requested_role IN ('worker', 'manager') THEN
    v_role := v_requested_role::public.user_role;
  ELSE
    v_role := 'admin'::public.user_role;
  END IF;

  -- Preserve existing profiles and never overwrite an existing non-null role.
  UPDATE public.profiles
  SET role = COALESCE(public.profiles.role, v_role),
      onboarding_step_completed = CASE
        WHEN public.profiles.role IS NULL AND v_role = 'admin'::public.user_role
...
-- Safe repair: promote likely self-signups that were created as worker.
